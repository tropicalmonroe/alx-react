...
describe("<Footer />", () => {
  ...
  it("Verify that the components at the least render the text “Copyright”", () => {
    const wrapper = shallow(<Footer />);
  ...
  it("verify that the link is displayed when the user is logged in within the context", () => {
    ...
    except(wrapper.find("div.footer a")).toHaveLength(1);
    except(wrapper.find("div.footer a").text()).toEqual("Contact us");
  });
});
